...
* 検索・絞り込み・ソートは、AGENTS.mdに従い本Issue #23および本ADRの対象外とし、Post-MVPの別Issueで扱う。
* Issue #27 / #29を実装する場合は、旧ADR-0010の契約をそのまま復活させず、本ADRで確定した統合画面の構造に合わせて、入力・出力・状態保持・再取得範囲を再設計する。
* 本ADRで扱うSession選択は、統合画面の表示対象を選ぶ操作であり、Asset一覧に対するSession条件の絞り込みとは区別する。
...
* Issue #29の絞り込みはPost-MVPの別スコープとしてADR-0010の共通契約から分離される。実装時には、本ADRで確定した統合画面の画面構造・Session選択・Usage遅延ロード境界に合わせて再設計する必要がある。
