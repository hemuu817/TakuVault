...
- Session統合画面専用のAsset詳細は作成しない。
- Asset詳細の表示方法はADR-0016 D5を正本とし、本ADRでは変更しない。
...
- Assetタイルから、既存のAsset詳細（`/assets/:id`）へ到達できる状態を維持する。
...
- ADR-0014：許可形式・判定方式・エラーハンドリング（認可済みAssetからのみURL生成）
